...
export class TwStateManager extends EventEmitter2 {
    ...
    onKill(e: TwEconEvent) {
        // FIXME: if victim is holding flag killer gets 2 points instead of 1
        ...
        // If cause of death isn't switching teams add to death counter
        if (e.weaponId !== -3) {
    ...
    async loadSettings() {
        ...
        for (const key in this.serverState.settings) {
            if (this.serverState.settings.hasOwnProperty(key)) {
                promises.push(
